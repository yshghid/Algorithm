...
- Breadth-First Search (BFS)를 사용하여 잔여 네트워크에서 가장 짧은 증가 경로를 선택하는 에드먼즈-카프 알고리즘은 이러한 문제를 해결한다.
- BFS를 사용하면 각 증가 경로는 최소 간선 수를 가지므로, 알고리즘이 더 빠르게 수렴한다.
- 에드먼즈-카프 알고리즘의 시간 복잡도는 O(VE^2)로, 퍼드 포커슨 알고리즘의 일반적인 시간 복잡도보다 더 효율적이다.
...
- 불변성 (invariant) 1: Δ 는 2의 거듭제곱 형태이다.
