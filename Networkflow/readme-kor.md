...
A: 한 번 에지에 흐름을 증가시키면 이를 다시 줄일 수 없기 때문이다. 이는 잘못된 경로를 선택했을 때 최적의 해결책을 찾을 수 없게 만든다. 

네트워크 흐름 그래프 G의 예시를 보면, 

예시. 흐름 네트워크 G를 고려해 봅시다.

유일한 최대 흐름 f 는 f(v, w) = 0을 가집니다.
그리디 알고리즘은 첫 번째 경로로 s→v→w→t를 선택할 수 있습니다.
요지. 잘못된 결정을 "되돌릴" 수 있는 메커니즘이 필요합니다.
