...
s --(6)--> u --(3)--> v --(6)--> t
  <--[4]--   <--[2]--   <--[2]--
```
- 현재 흐름 f는 [4, 2, 2] 이다이고 잔여 용량은 (6, 3, 6) 이다.
```
# 흐름을 3만큼 증가시킴
s --(3)--> u --(0)--> v --(3)--> t
  <--[7]--   <--[5]--   <--[5]--
```
- 흐름 f가 주어지고, P가 잔여 네트워크 Gf에서의 증가 경로일때, P를 따라 병목 용량 3만큼 흐름을 증가시키면, 결과 흐름 f'는 여전히 유효한 흐름이다.
```
# 흐름을 4만큼 증가시킴
s --(2)--> u --(-1)--> v --(2)--> t
  <--[8]--   <--[6]--   <--[6]--
```
- 만약 병목 용량보다 크게 4만큼 흐름을 증가시키면, 결과 흐름 f는 유효하지 않은 흐름이다. u->v 에지의 흐름이 에지의 용량인 5를 초과하여 용량 제약을 위반한다.

```
val(f') = val(f) + bottleneck(Gf,P)
```
- 새로운 흐름 f'의 값은 원래 흐름 f에 병목 용량을 더한 것과 같다.

### 최대 흐름 문제 - 퍼드 포커슨 알고리즘

<img src="https://github.com/yshghid/Algorithm/assets/153489198/2f2c8bb4-5213-4a4c-b36c-2763f9a64616" width=600>

그림에서 최대 흐름을 퍼드 포커슨 알고리즘으로 찾기.
...
- 병목 용량: min(10, 8, 10) = 8
- 경로를 따라 흐름이 8 증가: f(s->1) = 8, f(1->4) = 8, f(4->t) = 8
- 경로를 따라 흐름이 8 감소: f(1->2) = 2, f(4->1) = 0, f(t->4) = 2
- 흐름 f(s->t): 8
...
- 병목 용량: min(2, 2, 9, 2) = 2
- 경로를 따라 흐름이 2 증가: f(s->1) = 10, f(1->3) = 2, f(3->4)=2, f(4->t) = 10
- 경로를 따라 흐름이 2 감소: f(1->s) = 0, f(1->3) = 0, f(3->4)=7, f(4->t) = 0
- 흐름 f(s->t): 8+2=10
- 잔여 네트워크 업데이트
...
- 경로를 따라 흐름이 6 증가: f(s->3) = 6, f(3->4) = 8, f(4->2) = 6, f(2->t) = 6
- 경로를 따라 흐름이 6 감소: f(3->s) = 4, f(3->4) = 1, f(4->2) = 0, f(2->t) = 4
- 흐름 f(s->t): 10+6=16
- 잔여 네트워크 업데이트
...
- 네번째 증강 경로: s->3->1->2->t 
- 병목 용량: min(4,2,4,4) = 2
- 경로를 따라 흐름이 2 증가: f(s->3) = 8, f(3->1) = 2, f(1->2) = 2, f(2->t) = 8
- 경로를 따라 흐름이 2 감소: f(3->s) = 2, f(1->3) = 0, f(2->1) = 3, f(t->2) = 2
- 흐름 f(s->t): 16+2=18
- 잔여 네트워크 업데이트
```
...
cf) 네번째 업데이트의 효과: (3->1) 로 흐름을 2 증가시키는 것은 (1->3) 으로 흐름을 -2 증가시키는 것과 같기 때문에 두번째 업데이트에서 (1->3) 으로 흐름 2 증가되었던 것이 취소되었다.

- 다섯번째 증강 경로: s->3->4->1->2->t
- 병목 용량: min(2,1,8,,2) = 1
- 경로를 따라 흐름이 1 증가: f(s->3) = 9, f(3->4) = 9, f(4->1) = 1, f(1->2) = 3, f(2->t) = 9
- 경로를 따라 흐름이 1 감소: f(3->s) = 1, f(4->3) = 0, f(1->4) = 7, f(2->1) = 1, f(t->2) = 1
- 흐름 f(s->t): 18+1=19
- 잔여 네트워크 업데이트
```
...
cf) 다섯번째 업데이트의 효과: (4->1) 로 흐름을 1 증가시키는 것은 (1->4) 으로 흐름을 -1 증가시키는 것과 같기 때문에 첫번째 업데이트에서 (1->4) 으로 흐름 7 증가되었던 것이 1만큼 취소되었다.
